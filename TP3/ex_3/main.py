...
def main():
    ...
    multilayer_perceptron = MultiLayerPerceptron(learning_rate=0.2, hidden_layers=[], input_dim=2, output_dim=1, update_frequency=0,activation_function=np.vectorize(lambda x: np.sign(x)), activation_function_derivative=np.vectorize(lambda x: 1 ), update_learn_rate=None, scale_output=False)
