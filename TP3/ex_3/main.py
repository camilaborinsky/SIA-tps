...
def main():
    ...
    multilayer_perceptron = MultiLayerPerceptron(learning_rate=0.1, hidden_layers=[2], input_dim=2, output_dim=1, activation_function=np.vectorize(lambda x: np.sign(x)), activation_function_derivative=np.vectorize(lambda x: 1 ), update_learn_rate=None)
