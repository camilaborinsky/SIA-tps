...
def main():
    ...
    multilayer_perceptron = MultiLayerPerceptron(learning_rate=0.2, hidden_layers=[], input_dim=len(training[0]), output_dim=len(expected[0]), activation_function=(lambda x: 1/(1+np.exp(-2*x))), activation_function_derivative=(lambda x: 2*(1/(1+np.exp(-2*x)))*(1-(1/(1+np.exp(-2*x)))) ), update_learn_rate=None, scale_output=True)
